...
public function klaim()
{
    ...
    $historyGift = KlaimPoinModel::where('user_id', $user->id)->get();

    $menunggu = 0;

    foreach($historyGift as $gift) {
        if($gift->status == 'Menunggu') {
            $menunggu++;
        } 
    }

    // Kemudian kirimkan data ke tampilan menggunakan view()
    return view('member.klaim', compact('gifts', 'user', 'menunggu'));
